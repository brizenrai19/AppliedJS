...
function carouselCreator(carouselItem){
  const divCarousel = document.createElement("div");
  divCarousel.className = "carousel";
  const leftButton = document.createElement("div");
  leftButton.className = "left-button";
  leftButton.innerHTML = "<";
  divCarousel.appendChild(leftButton);
  imageSources.forEach(imgSrc =>{
    const imgElement = document.createElement("img");
    imgElement.src = imgSrc;
    divCarousel.appendChild(imgElement);
  })
  const rightButton = document.createElement("div");
  rightButton.innerHTML = ">";
  rightButton.className= "right-button";
  divCarousel.appendChild(rightButton);
  const mainCarouselDiv = document.getElementsByClassName("errors-container")[0];
  mainCarouselDiv.appendChild(divCarousel);
}
